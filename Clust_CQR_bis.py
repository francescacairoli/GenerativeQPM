...
class ClustCQR():
	...
	'''
	def __init__(self, Lc, Xc, cal_loader, num_cal_points, stl_property, cp_classifier, trained_generator, num_classes, opt, quantiles = [0.05, 0.95], plots_path=''):
	...
	def get_pred_interval(self, inputs_loader,extra=''):
		...
		for i in range(gen_trajs.shape[0]):
			...
			rescaled_trajs_i[:,:int(-self.opt.testmissingratio)] = rescaled_realtrajs_i[:,:int(-self.opt.testmissingratio)]
	...
	def get_scores_threshold(self):
		...
		self.calibr_quantiles = self.get_pred_interval(self.cal_loader,extra='cal_')
		...
		for g in range(self.num_classes):
			...
			if len(self.calibr_scores[g]) > 0:
				...
				plt.tight_layout()
				plt.savefig(self.plots_path+f'/calibr_nonconf_scores_cluster_g={g}.png')
				plt.close()
			else:
				self.tau[g] = math.inf
	...
	def plot_multimodal_errorbars(self, y, qr_interval, cqr_interval, title_string, plot_path, extra_info = ''):
		...
		plt.grid(True)
		plt.tight_layout()
		fig.savefig(plot_path+"/cluster_"+extra_info+"_multimodal_errorbar.png")
		plt.close()
