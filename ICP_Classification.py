...
class ICP_Classification():
	...
	def set_calibration_scores(self):
		...
		cal_centroids, _ = self.trained_model.evaluate(self.Xc)

		# distance of every calibr trajectory to its assigned centroid
		cal_dist = np.linalg.norm(cal_centroids - self.Xc, ord=2, axis=(1,2)) #[n_cal_points*num_cal_trajs,]
	...
	def get_nonconformity_scores(self, x, class_index):
		...
		centroids = self.trained_model.centroids

		dist = np.linalg.norm(centroids[class_index] - x, ord=2, axis=(1,2)) 
