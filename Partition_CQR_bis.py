...
class PartitionCQR():
	...
	def __init__(self, Lc, Xc, cal_loader, num_cal_points, stl_property, partition_fnc, trained_generator, num_classes, opt, quantiles = [0.05, 0.95], plots_path=''):
		...
		self.Xc = Xc # [nb_state*nb_trajs, L,K]
		self.partition_fnc = partition_fnc
	...
	def get_scores_threshold(self):
		...
		#calibr_class_quantiles
		self.calibr_quantiles = self.get_pred_interval(self.cal_loader,extra='cal_') # num_classes, nb_states, 2
	...
	def get_cpi(self, tloader, pi_flag = False):
		'''
		...
		pis = self.get_pred_interval(tloader,extra='test_') #[n_points,n_classesm,2]
	...
	def plot_errorbars(self, y, qr_interval, cqr_interval, title_string, plot_path, extra_info = ''):
		...
		'''
		n_points_to_plot = 30
		print('TEST ROBS: ', np.mean(y), np.min(y), np.max(y))
