...
if args.model_name == 'crossroad':
	if args.property_idx == 3:
		stl_fnc_cal = lambda trajs: eval_crossroad_multiagent_property(trajs, ped_gen, ped_cal_loader, prop_idx=3, extra='full',nsample=args.nsample)
		stl_fnc_test = lambda trajs: eval_crossroad_multiagent_property(trajs, ped_gen, ped_test_loader, prop_idx=3, extra='full',nsample=args.nsample)
		stl_fnc_calbatch = lambda trajs: eval_crossroad_multiagent_property(trajs, ped_gen, ped_calbatch_loader, prop_idx=3, extra='batch',nsample=1)
		stl_fnc_testbatch = lambda trajs: eval_crossroad_multiagent_property(trajs, ped_gen, ped_testbatch_loader, prop_idx=3, extra='batch',nsample=1)
...
if not args.calload:
	...
	cqr.plot_multimodal_errorbars(Rtest_res, pis, cpis, 'unimodal cqr - baseline', foldername, extra_info=str(args.property_idx)+f'_class={args.classifier}', model_name=args.model_name)
	...
	cqr.plot_multimodal_errorbars(D['Rtest_res'], D['pis'], D['cpis'], 'unimodal cqr - baseline', foldername, extra_info=str(args.property_idx), model_name=args.model_name)
